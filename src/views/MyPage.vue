        ...
        <v-dialog v-model="showConfirmDialog" max-width="500px">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <!-- 확인 문구가 일치할 때만 탈퇴 작업 실행 -->
                    <v-btn color="red" text @click="confirmWithdrawal">탈퇴하기</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
  ...
  methods: {
    ...
    cancelReservation(reservationNum) {
      axios.delete(`/reservations/${reservationNum}`)
        ...
        confirmWithdrawal() {
            // 사용자가 입력한 확인 문구가 '회원탈퇴'와 일치하는지 확인
            if (this.confirmationText === "회원탈퇴") {
                // 일치하면 회원 탈퇴 로직을 실행
                this.performWithdrawal();
            } else {
                // 일치하지 않으면 에러 메시지를 표시하거나 다른 조치를 취할 수 있음
                alert("확인 문구가 일치하지 않습니다. 다시 시도해주세요.");
            }
        },
        performWithdrawal() {
            // 회원 탈퇴 처리를 진행하는 메서드
